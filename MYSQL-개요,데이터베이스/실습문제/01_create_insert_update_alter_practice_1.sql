...
-- order 는 sql 예약어라서 보통 테이블 명칭으로는 사용할 수 없음. 
CREATE TABLE `ORDER` (
...
-- update 문을 sql에서 직접적으로 실행할 때 문제가 되는것을 방지하기 위해 safe 모드 생성
-- 테이블을 삭제할 때도 마찬가지로 safe 모드이므로 해지후 변경 가능

set sql_safe_updates = 0;
update user set password_hash = 'hong123'
where username = '홍길동'; -- 안전모드 상태로 바로 바꿀 수 없음
...
-- alter table 수정할 테이블명칭 modify column 컬럼명 컬럼명의 규칙
 ...
 -- 꼬였을 때 테이블 삭제 방법. 
 -- 외래키 체크 종료 (종료 =0 시작은 = 1)
 set foreign_key_checks = 0;
 drop table if exists 테이블 명칭
 
 
 -- 만약 menu 테이블이 잘못되었을 꺠
 --  drop table if exists menu;
 
 -- 테이블 삭제후 외래키 체크 시작 설정
